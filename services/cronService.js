...
exports.actualizarEstadosCampeonatos = async () => {
  try {
    ...
    if (nuevoEstado === campeonatoEstados.transaccionProceso) {
      ...
      `,
        {
          replacements: {
            ...
            fechaReferencia: moment(campeonato.fecha_registro).format(
              "YYYY-MM-DD HH:mm:ss",
            ),
          },
      ...
      const ascensosVarones = await campeonatoService.getEquiposAscensoDescenso(
        campeonatoAnterior.id,
        "V",
      );
      ascensosVarones.forEach((cambio) => {
        const equipoId = cambio.equipo.equipo_id;
        const nuevaCategoria = categorias.find(
          (cat) => cat.nombre === cambio.a && cat.genero === "V",
        );
        if (nuevaCategoria) {
      ...
      const ascensosMujeres = await campeonatoService.getEquiposAscensoDescenso(
        campeonatoAnterior.id,
        "D",
      );
      ascensosMujeres.forEach((cambio) => {
        const equipoId = cambio.equipo.equipo_id;
        const nuevaCategoria = categorias.find(
          (cat) => cat.nombre === cambio.a && cat.genero === "D",
        );
        if (nuevaCategoria) {
          cambiosCategoria.set(equipoId, nuevaCategoria.id);
        }
      ...
      const idsYaRegistrados = new Set(
        equiposYaRegistrados.map((e) => e.equipoId),
      );
      ...
      for (const equipo of equiposAnteriorCampeonato) {
        ...
        const nuevaCategoriaId =
          cambiosCategoria.get(equipoId) || equipo.categoria_id;
      ...
      if (nuevosRegistrosConCategoria.length > 0) {
        ...
        console.log(
          "No se encontraron equipos del campeonato anterior para insertar.",
        );
      }
    }
