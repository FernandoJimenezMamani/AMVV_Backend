...
exports.submitResultados = async (req, res) => {
  ...
  if (!partido_id || !resultadoLocal || !resultadoVisitante || !imagenPlanilla) {
    return res.status(400).json({ message: 'Todos los campos requeridos deben ser proporcionados' });
  }
...
exports.generarFixture = async (req, res) => {
  ...
  if (!campeonatoId || !categoriaId) {
    return res.status(400).json({ message: 'Se requiere el ID del campeonato y la categoría.' });
  }

  try {
    const equipos = await partidoService.getEquiposInscritos(campeonatoId, categoriaId);

    if (equipos.length < 2) {
      return res.status(400).json({ message: 'No hay suficientes equipos inscritos para generar partidos.' });
    }
    ...
    res.status(500).json({ message: 'Error al generar el fixture.', error: error.message });
  }
};
...
exports.generarFixtureConFechas = async (req, res) => {
  ...
  try {
    ...
    if (!fixture || fixture.length === 0) {
      return res.status(400).json({ message: 'Error: No se pudo generar el fixture.' });
    }

    console.log("Fixture generado:", fixture); // 📌 Depuración

    const fechasSuficientes = partidoService.validarDisponibilidadFechas(fixture, campeonato);
    if (!fechasSuficientes) {
      return res.status(400).json({ message: 'El rango de fechas del campeonato no permite jugar todos los partidos. Amplía el rango de fechas.' });
    }

    const partidosConFechas = await partidoService.asignarFechasHorarios(fixture, campeonato);

    if (!partidosConFechas || partidosConFechas.length === 0) {
      return res.status(400).json({ message: 'Error: No se pudo asignar fechas a los partidos.' });
    }

    res.status(200).json({ partidos: partidosConFechas });
  } catch (error) {
    console.error("Error en la generación de partidos:", error); // 📌 Depuración
    res.status(500).json({ message: 'Error al generar los partidos con fechas.', error: error.message });
  }
};
...
exports.generarFixtureCompleto = async (req, res) => {
  ...
  try {
    ...
    let partidosConFechas = await partidoService.asignarFechasHorarios(fixture, campeonato);
    let partidosConLugares = await partidoService.asignarLugaresAPartidos(partidosConFechas);
    let partidosConArbitros = await partidoService.asignarArbitrosAPartidos(partidosConLugares);
    
    res.status(200).json({ partidos: partidosConArbitros });
  } catch (error) {
    console.error("🚨 Error en la generación del fixture completo:", error);
    res.status(500).json({ message: 'Error al generar los partidos con árbitros.', error: error.message });
  }
};
...
exports.registrarPartidos = async (req, res) => {
  ...
  try {
    const resultado = await partidoService.registrarPartidos(partidos, campeonatoId, categoriaId);
    res.status(201).json(resultado);
  } catch (error) {
    console.error("🚨 Error en el registro de partidos:", error);
    res.status(400).json({ message: error.message || "Error al registrar los partidos." });
  }
};
...
exports.getPartidosByCampeonato = async (req, res) => {
  const { campeonatoId } = req.params;
    ...
    try {
        ...
        const partidos = await partidoService.getPartidosByCampeonatoYFecha(campeonatoId, fecha);

        if (partidos.length === 0) {
            return res.status(404).json({ message: 'No se encontraron partidos para este campeonato en la fecha indicada' });
        }
        ...
        // Configurar la respuesta para abrir el PDF en el navegador
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `inline; filename="${campeonatoNombre}_${fecha}.pdf"`);
        res.sendFile(path.resolve(pdfPath));
