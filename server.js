...
const {
  actualizarEstadosCampeonatos,
  monitorearJugadoresParticipacion,
...
const broadcastMessage = (message) => {
  ...
  clients.forEach((ws, clientId) => {
    if (ws.readyState === WebSocket.OPEN) {
      ...
      console.log(
        `Cliente ${clientId} no está en estado OPEN, no se envió el mensaje.`,
      );
    }
  });
};
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (message) => {
    try {
      ...
      console.error(
        `Error procesando mensaje del cliente ${clientId}:`,
        message,
        error,
      );
    }
  ...
  ws.on("close", (code, reason) => {
    console.log(
      `Cliente ${clientId} desconectado. Código: ${code}, Razón: ${reason || "Sin razón proporcionada"}`,
    );
